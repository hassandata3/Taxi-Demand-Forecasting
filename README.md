...
*(computed from the full 18-month analytical dataset — Jan 2025 to Jun 2026, all zones and hours)*
...
*(model evaluation on the test period only — 18 Feb to 30 Jun 2026, matching the dashboard)*

| Model | MAE | RMSE | R² |
|---|---|---|---|
| Baseline (same hour last week) | 3.66 | 5.24 | 0.49 |
| XGBoost | 2.60 | 3.75 | 0.74 |

29% lower error than the baseline. The model underpredicts slightly (−0.5 trips/hour on average) because test-period demand kept growing past what it saw in training — a stated limitation, not a hidden one.
